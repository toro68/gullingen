...
def vis_hyttegrend_aktivitet():
    ...
    try:
        ...
        df_aktivitet = pd.DataFrame(index=dato_range).copy()  # Lag en eksplisitt kopi
        ...
        with debug_container.expander("🔍 Debug Info", expanded=False):
            ...
            try:
                ...
                st.write("Prosessert data:", {
                    ...
                    "Siste bestilling": df['avreise'].iloc[0] if not df.empty else None
                })
                
            except Exception as e:
                logger.error(f"Error processing dates: {str(e)}")
                st.error(f"Feil ved prosessering av datoer: {str(e)}")
                df['ankomst'] = pd.NaT
                df['avreise'] = pd.NaT
