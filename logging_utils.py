...
def log_status_change(bestilling_id, old_status, new_status, changed_by, db_name='stroing'):
    """
    ...
    try:
        ...
        if rows_affected > 0:
            ...
            logger.warning(f"Ingen rader påvirket ved logging av statusendring for bestilling {bestilling_id}")
            return False
    
    except Exception as e:
        logger.error(f"Feil ved logging av statusendring for bestilling {bestilling_id}: {str(e)}")
        return False
...
def log_general_event(event_type, description, user_id=None, db_name='event_log'):
    ...
    try:
        ...
        if rows_affected > 0:
            ...
            logger.warning(f"Ingen rader påvirket ved logging av hendelse: {event_type}")
            return False
