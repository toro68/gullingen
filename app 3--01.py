...
def validate_data(data):
    ...
    data = np.array(data, dtype=float)  # Ensure float type
    data[data < -100] = np.nan  # Assume values below -100 are invalid
...
def fetch_and_process_data(client_id, date_start, date_end):
    ...
    try:
        ...
        # Calculate snow drift alarms
        df['snow_drift_alarm'] = ((df['wind_speed'] > 6) & 
                                  (df['air_temperature'] <= -1) & 
                                  (df['precipitation_amount'] < 0.1) & 
                                  (df['surface_snow_thickness'].diff().abs() >= 1)).astype(int)

        # Calculate slippery road alarms
        df['slippery_road_alarm'] = ((df['air_temperature'] > 0) & 
                                     (df['precipitation_amount'] > 1.5) & 
                                     (df['surface_snow_thickness'] >= 20) & 
                                     (df['surface_snow_thickness'].diff() < 0)).astype(int)
...
# Oppdatert funksjon for å beregne snøfokk-alarmer
def calculate_snow_drift_alarms(df):
    ...
    df['snow_drift_alarm'] = (
        ...
        (df['air_temperature'] <= -1) & 
        (
            ((df['precipitation_amount'] < 0.1) & (df['snow_depth_change'].abs() >= 1)) |
            ((df['precipitation_amount'] >= 0.1) & (df['snow_depth_change'] <= -0.5))
        )
    ).astype(int)
    logger.info("Completed function: calculate_snow_drift_alarms")
    return df

# Oppdatert funksjon for å beregne glatt vei / slush-alarmer
def calculate_slippery_road_alarms(df):
    logger.info("Starting function: calculate_slippery_road_alarms")
    df['slippery_road_alarm'] = (
        (df['air_temperature'] > 0) & 
        (df['precipitation_amount'] > 1.5) & 
        (df['surface_snow_thickness'] >= 20) & 
        (df['surface_snow_thickness'].diff() < 0)
    ).astype(int)
    logger.info("Completed function: calculate_slippery_road_alarms")
    return df

# Oppdatert funksjon for å hente og behandle værdata
@st.cache_data(ttl=3600)
def fetch_and_process_data(client_id, date_start, date_end):
    logger.info("Starting function: fetch_and_process_data")
    try:
