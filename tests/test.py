...
class TestWeatherApp(unittest.TestCase):
    ...
    def test_save_feedback(self):
        result = app.save_feedback(
            "Test Type",
            "2023-09-22T12:00:00+02:00",
            "Test Comment",
            "Test User",
            self.conn_feedback,
        )
    ...
    def test_authenticate_user(self):
        ...
        app.st.secrets = {"auth_codes": {"users": {"Test User": "test_code"}}}
    ...
    def test_lagre_bestilling(self):
        app.lagre_bestilling(
            ...
            "12:00",
            "Ukentlig",
            self.conn_tunbroyting,
        )

        # Verify the booking was saved
        df = pd.read_sql_query(
            "SELECT * FROM tunbroyting_bestillinger", self.conn_tunbroyting
        )
        self.assertEqual(len(df), 1)
        self.assertEqual(df.iloc[0]["bruker"], "Test User")

    def test_lagre_stroing_bestilling(self):
        app.lagre_stroing_bestilling(
            "Test User", "2023-09-22", "Test Comment", self.conn_stroing
        )
    ...
    def test_update_stroing_status(self):
        ...
        df = pd.read_sql_query(
            "SELECT * FROM stroing_bestillinger WHERE id = 1", self.conn_stroing
        )
